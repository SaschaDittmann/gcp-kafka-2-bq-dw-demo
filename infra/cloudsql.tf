...
resource "google_sql_database" "chinook" {
  ...
  provisioner "local-exec" {
    when    = destroy
    command = "${path.module}/../scripts/destroy-cleanup.sh ${self.instance} ${self.name} ${self.project}"
  }

  # Ensure chinook is destroyed (provisioner runs) BEFORE the managed_kafka_iam
  # user. Without this, both are deleted in parallel and the user deletion
  # fails with "objects depend on it".
